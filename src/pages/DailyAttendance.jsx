...
import { Search, ChevronDown, ChevronUp, User, Save, ChevronLeft, ChevronRight, Lock, Printer, Edit3, FileText } from 'lucide-react';
...
export default function DailyAttendance({ employees, logHistory }) {
  ...
  return (
    <div className="h-full">
      <div className={`space-y-8 animate-in fade-in duration-500 ${printMode === 'matrix' || printMode === 'summary' ? 'hidden' : 'block'}`}>
        ...
        <div className={`flex-col md:flex-row justify-between items-end gap-6 ${printMode === 'dtr' ? 'hidden' : 'flex'}`}>
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            <div className="relative w-full md:w-56 mr-2">
            ...
            <button onClick={() => triggerPrint('summary')} className="flex items-center justify-center gap-2 px-4 py-3 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200">
                              ...
                              <div className={`bg-white max-w-3xl mx-auto ${printMode === 'dtr' ? 'p-0' : 'shadow-2xl border border-slate-200 p-10 md:p-14'} min-h-[1000px]`}>
                                ...
                                <div className="flex items-start gap-5 mb-4">
                                  ...
                                  <div>
                                    ...
                                    <p className="font-bold tracking-[0.3em] text-[10px] uppercase mt-2">Electronic & Electrical Services</p>
      ...
      {/* --- PRINT ONLY VIEW: PAYROLL SUMMARY --- */}
      {printMode === 'summary' && (
        <div className="block text-black bg-white p-8 font-sans max-w-none">
              ...
              {employees.map((emp) => {
                ...
                return (
                  <tr key={emp.id}>
                    ...
                    <td className="border border-black p-3 text-center font-bold text-base">{totalPresent}</td>
                    <td className="border border-black p-3 text-center font-bold text-base">{totalLeave}</td>
                    <td className="border border-black p-3 text-center font-bold text-base">{totalNoWork}</td>
                  </tr>
                );
              })}
            </tbody>
