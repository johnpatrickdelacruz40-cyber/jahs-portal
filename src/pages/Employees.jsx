...
export default function Employees({ employees, setEmployees, logHistory }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (editingId) {
      ...
      logHistory(`Updated record for ${formData.lastName}`);
    } else {
      setEmployees([...employees, { ...formData, id: Date.now() }]);
      logHistory(`Added new record: ${formData.lastName}`);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => openModal()} className="flex items-center px-5 py-2.5 bg-slate-900 text-white rounded-xl font-bold text-sm">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="p-5 bg-slate-50/50 border-b border-slate-100">
          <div className="relative max-w-md">
            ...
            <input 
              type="text" placeholder="Search ID or Last Name..." value={searchTerm}
            ...
            <tbody className="divide-y divide-slate-50">
              {employees.filter(e => e.lastName.toLowerCase().includes(searchTerm.toLowerCase())).map((emp) => (
                <tr key={emp.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 font-mono text-xs text-slate-500">{emp.idNo}<br/>{emp.sec}</td>
                  <td className="px-6 py-4 font-bold text-slate-800">{emp.lastName}, {emp.firstName}</td>
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    ...
                    <button onClick={() => { setEmployees(employees.filter(e => e.id !== emp.id)); logHistory(`Deleted record: ${emp.lastName}`); }} className="text-slate-400 hover:text-rose-600"><Trash2 size={14} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-6 border-b flex justify-between items-center bg-slate-50">
              <h2 className="text-lg font-bold">{editingId ? 'Edit Personnel' : 'New Personnel'}</h2>
              <button onClick={() => setIsModalOpen(false)}><X size={20} /></button>
            </div>
            <form onSubmit={handleSubmit} className="p-8 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2 flex justify-center mb-4">
                <div onClick={() => fileInputRef.current.click()} className="w-32 h-32 border-2 border-dashed border-slate-300 rounded-xl flex flex-col items-center justify-center cursor-pointer overflow-hidden bg-slate-50">
                  {formData.photo ? <img src={formData.photo} className="w-full h-full object-cover" /> : <><Camera size={24} className="text-slate-300"/><span className="text-[10px] text-slate-400 mt-2">UPLOAD 2X2</span></>}
                </div>
              ...
              {Object.keys(formData).map((key) => key !== 'photo' && key !== 'id' && (
                <div key={key} className="space-y-1">
                  ...
                  {key === 'status' ? (
                    <select value={formData[key]} onChange={(e) => setFormData({...formData, [key]: e.target.value})} className="w-full px-4 py-2 bg-slate-50 border rounded-lg outline-none">
              ...
              <div className="md:col-span-2 pt-6 flex justify-end gap-3 border-t">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 py-2 text-xs font-bold text-slate-400">Discard</button>
                <button type="submit" className="px-10 py-2 bg-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-100 transition-all">Save Record</button>
              </div>
            </form>
          </div>
        </div>
